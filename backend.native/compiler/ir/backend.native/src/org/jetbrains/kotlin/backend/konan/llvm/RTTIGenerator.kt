...
internal class RTTIGenerator(override val context: Context) : ContextUtils {
    ...
    private val arrayClasses = mapOf(
            (StandardNames.BUILT_INS_PACKAGE_FQ_NAME to "Array")              to kObjHeaderPtr,
            (StandardNames.BUILT_INS_PACKAGE_FQ_NAME to "ByteArray")          to int8Type,
            (StandardNames.BUILT_INS_PACKAGE_FQ_NAME to "CharArray")          to int16Type,
            (StandardNames.BUILT_INS_PACKAGE_FQ_NAME to "ShortArray")         to int16Type,
            (StandardNames.BUILT_INS_PACKAGE_FQ_NAME to "IntArray")           to int32Type,
            (StandardNames.BUILT_INS_PACKAGE_FQ_NAME to "LongArray")          to int64Type,
            (StandardNames.BUILT_INS_PACKAGE_FQ_NAME to "FloatArray")         to floatType,
            (StandardNames.BUILT_INS_PACKAGE_FQ_NAME to "DoubleArray")        to doubleType,
            (StandardNames.BUILT_INS_PACKAGE_FQ_NAME to "BooleanArray")       to int8Type,
            (StandardNames.BUILT_INS_PACKAGE_FQ_NAME to "String")             to int16Type,
            (KonanFqNames.packageName to "ImmutableBlob")                     to int8Type,
            (KonanFqNames.internalPackageName to "NativePtrArray")            to kInt8Ptr
    )
    ...
    private fun getInstanceSize(classType: LLVMTypeRef?, irClass: IrClass) : Int {
        val elementType = arrayClasses.get(irClass.packageFqName to irClass.name.asString())
    ...
    private fun makeExtendedInfo(irClass: IrClass): ConstPointer {
        ...
        val llvmDeclarations = context.llvmDeclarations.forClass(irClass)
        val bodyType = llvmDeclarations.bodyType
        val elementType = arrayClasses[irClass.packageFqName to irClass.name.asString()]
